...
import { getFirestore, doc, onSnapshot, setDoc, updateDoc, collection, query, where, getDocs, writeBatch, serverTimestamp } from 'https://www.gstatic.com/firebasejs/12.2.1/firebase-firestore.js';
...
async function save(open=false) {
...
q('openBtn').onclick = () => save(true);
q('closeBtn').onclick = () => updateDoc(sessionRef(),{isOpen:false,updatedAt:serverTimestamp()});
...
q('resetBtn').onclick = async () => {
  const snap = await getDocs(query(collection(db,'responses'),where('sessionId','==',currentSession)));
  const b = writeBatch(db);
  snap.forEach(x=>b.delete(x.ref));
  await b.commit();
  const opts = options();
  await updateDoc(sessionRef(),{counts:opts.map(()=>0),showResults:false,roundId:crypto.randomUUID(),updatedAt:serverTimestamp()});
};

async function loadWords(roundId) {
  if (!roundId) return render(null,[]);
  const snap = await getDocs(query(collection(db,'responses'),where('sessionId','==',currentSession),where('roundId','==',roundId)));
  render({type:'wordcloud'},snap.docs.map(x=>x.data().text||''));
}
...
function render(d,words) {
  if (d?.type === 'wordcloud') {
    const freq = {};
    words.forEach(w => {
      const k = w.trim().toLocaleLowerCase('it');
